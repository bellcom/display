...
print "\nApache2 conf\n";
$systemcmd = "wget -q --output-document=/etc/apache2/sites-available/".$sitename.".conf " . $vhostaddress . "";
if ($test) {
  print "Udfører(Debug): \n" . $systemcmd . "\n";
}
else {
...
$systemcmd = "perl -pi -e 's/\[domain\]/".$sitename."/g' /etc/apache2/sites-available/".$sitename.".conf";
if ($test) {
  print "Udfører(Debug): \n" . $systemcmd . "\n";
}
else {
...
if ($using_varnish == 2) {
  $systemcmd = "perl -pi -e 's/\*:80\>/\*:8080\>/g' /etc/apache2/sites-available/".$sitename.".conf";
  if ($test) {
    print "Udfører(Debug): \n" . $systemcmd . "\n";
  }
  else {
...
# Enabler vhost
$systemcmd = "a2ensite " .$sitename.".conf";
if ($test) {
  print "Udfører(Debug): \n" . $systemcmd . "\n";
}
else {
...
$mail = system("logname") . "@bellcom.dk";
mail($mail,"New site at $hostname","DBNAME = $dbname DBUSERNAME = $dbusername DBPASS = $passwd\nSitepath: $sitepath$sitename\n");
